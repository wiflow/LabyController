...
public class InputHandler {
    ...
    // Cooldown before null state deactivates controller (prevents flickering)
    private int nullStateDeactivateCooldown = 0;
    private static final int NULL_STATE_COOLDOWN_TICKS = 20;
    ...
    public void update() {
        ...
        if (nullStateDeactivateCooldown > 0) {
            nullStateDeactivateCooldown--;
        }

        if (state == null) {
            // Don't immediately deactivate - wait for cooldown to prevent flickering
            if (nullStateDeactivateCooldown <= 0 && controllerActive) {
                controllerActive = false;
            }
            return;
        }

        // State is valid - reset the null state cooldown
        nullStateDeactivateCooldown = NULL_STATE_COOLDOWN_TICKS;
