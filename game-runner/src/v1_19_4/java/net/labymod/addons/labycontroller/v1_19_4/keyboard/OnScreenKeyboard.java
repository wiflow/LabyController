...
public class OnScreenKeyboard extends Screen {
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        fill(poseStack, 4, inputY - 2, this.width - 4, inputY + inputHeight, 0xF0222222);
        fill(poseStack, 4, inputY - 2, this.width - 4, inputY - 1, 0xFF444444);
        fill(poseStack, 4, inputY + inputHeight - 1, this.width - 4, inputY + inputHeight, 0xFF111111);
        ...
        if (targetEditBox != null) {
            ...
            font.draw(poseStack, displayText, 10, inputY + 6, textColor);

            if (!text.isEmpty() && (System.currentTimeMillis() / 500) % 2 == 0) {
                int cursorX = 10 + font.width(prefix + text);
                fill(poseStack, cursorX, inputY + 4, cursorX + 1, inputY + 16, 0xFFFFFFFF);
            }
        }
        ...
        fill(poseStack, 0, keyboardY, this.width, this.height, 0xF0111111);
        fill(poseStack, 0, keyboardY, this.width, keyboardY + 1, 0xFF333333);
        ...
        for (int row = 0; row < keyData.size(); row++) {
            ...
            for (int col = 0; col < rowKeys.size(); col++) {
                ...
                renderKey(poseStack, key, selected);
            }
        }

        renderButtonLegend(poseStack);

        String hint = "Use left stick to navigate, press buttons for shortcuts";
        drawCenteredString(poseStack, font, hint, this.width / 2, 22, 0xFF888888);
    }

    private void renderButtonLegend(PoseStack poseStack) {
        int y = 6;
        int x = 10;

        String btnA = getButtonLabel("A");
        renderButtonIcon(poseStack, btnA, getButtonColor("A"), x, y);
        font.draw(poseStack, "Select", x + 18, y + 3, 0xFFCCCCCC);
        x += 60;

        String btnX = getButtonLabel("X");
        renderButtonIcon(poseStack, btnX, getButtonColor("X"), x, y);
        font.draw(poseStack, "Delete", x + 18, y + 3, 0xFFCCCCCC);
        x += 60;

        String btnY = getButtonLabel("Y");
        renderButtonIcon(poseStack, btnY, getButtonColor("Y"), x, y);
        font.draw(poseStack, "Space", x + 18, y + 3, 0xFFCCCCCC);
        x += 55;

        String btnLB = getButtonLabel("LB");
        renderButtonIcon(poseStack, btnLB, getButtonColor("LB"), x, y);
        font.draw(poseStack, "Shift", x + 22, y + 3, 0xFFCCCCCC);
        x += 55;

        renderButtonIcon(poseStack, "≡", 0xFF666666, x, y);
        font.draw(poseStack, "Enter", x + 18, y + 3, 0xFFCCCCCC);
        x += 55;

        String btnB = getButtonLabel("B");
        renderButtonIcon(poseStack, btnB, getButtonColor("B"), x, y);
        font.draw(poseStack, "Close", x + 18, y + 3, 0xFFCCCCCC);
    }
    ...
    private void renderKey(PoseStack poseStack, KeyData key, boolean selected) {
        int x = key.x;
        int y = key.y;
        int w = key.width;
        int h = key.height;
        ...
        fill(poseStack, x + 2, y + 2, x + w - 2, y + h - 2, bgColor);

        fill(poseStack, x + 2, y, x + w - 2, y + 2, highlightColor);
        fill(poseStack, x + 2, y + h - 2, x + w - 2, y + h, shadowColor);
        fill(poseStack, x, y + 2, x + 2, y + h - 2, highlightColor);
        fill(poseStack, x + w - 2, y + 2, x + w, y + h - 2, shadowColor);

        if (selected) {
            fill(poseStack, x - 1, y + 2, x, y + h - 2, 0xFF4488DD);
            fill(poseStack, x + w, y + 2, x + w + 1, y + h - 2, 0xFF4488DD);
            fill(poseStack, x + 2, y - 1, x + w - 2, y, 0xFF4488DD);
            fill(poseStack, x + 2, y + h, x + w - 2, y + h + 1, 0xFF4488DD);
        }
        ...
        if (key.shortcut != null) {
            ...
            renderButtonIcon(poseStack, buttonLabel, buttonColor, iconX, iconY);

            int textX = iconX + (buttonLabel.length() > 1 ? 24 : 20);
            font.draw(poseStack, displayText, textX, y + (h - 8) / 2, textColor);
        } else {
            drawCenteredString(poseStack, font, displayText, x + w / 2, y + (h - 8) / 2, textColor);
        }
    }

    private void renderButtonIcon(PoseStack poseStack, String label, int color, int x, int y) {
        int size = 14;

        if (label.equals("LB") || label.equals("RB")) {
            int w = 20;
            fill(poseStack, x + 1, y, x + w - 1, y + size, color);
            fill(poseStack, x, y + 1, x + w, y + size - 1, color);
            fill(poseStack, x + 1, y + 1, x + w - 1, y + 2, lightenColor(color, 40));
            drawCenteredString(poseStack, font, label, x + w / 2, y + 3, 0xFFFFFFFF);
        } else if (label.equals("≡")) {
            fill(poseStack, x + 2, y + 2, x + size - 2, y + size - 2, color);
            fill(poseStack, x + 1, y + 3, x + size - 1, y + size - 3, color);
            fill(poseStack, x + 3, y + 1, x + size - 3, y + size - 1, color);
            fill(poseStack, x + 4, y + 4, x + size - 4, y + 5, 0xFFFFFFFF);
            fill(poseStack, x + 4, y + 6, x + size - 4, y + 7, 0xFFFFFFFF);
            fill(poseStack, x + 4, y + 8, x + size - 4, y + 9, 0xFFFFFFFF);
        } else {
            fill(poseStack, x + 3, y, x + size - 3, y + size, color);
            fill(poseStack, x + 2, y + 1, x + size - 2, y + size - 1, color);
            fill(poseStack, x + 1, y + 2, x + size - 1, y + size - 2, color);
            fill(poseStack, x, y + 3, x + size, y + size - 3, color);

            fill(poseStack, x + 3, y + 1, x + size - 3, y + 2, lightenColor(color, 50));
            fill(poseStack, x + 4, y + 2, x + size - 4, y + 3, lightenColor(color, 30));

            fill(poseStack, x + 3, y + size - 2, x + size - 3, y + size - 1, darkenColor(color, 40));

            drawCenteredString(poseStack, font, label, x + size / 2, y + 3, 0xFFFFFFFF);
        }
    }
