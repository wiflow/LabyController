package net.labymod.addons.labycontroller.v1_19_4.radial;
...
public class RadialMenuScreen extends Screen {
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        for (int i = 0; i < SLOT_COUNT; i++) {
            ...
            drawSlot(poseStack, (int) x, (int) y, actions[i], isSelected, i + 1);
        }

        // Draw center indicator
        int indicatorSize = (int) (8 * animationProgress);
        fill(poseStack, centerX - indicatorSize, centerY - indicatorSize,
        ...
        if (selectedSlot >= 0 && selectedSlot < actions.length) {
            ...
            drawCenteredString(poseStack, font, name, centerX, centerY + (int) RADIUS + 30, 0xFFFFFFFF);
        }

        // Draw hint at top
        drawCenteredString(poseStack, font, "Use right stick to select, release to confirm", centerX, 20, 0xFFAAAAAA);
    }

    private void drawSlot(PoseStack poseStack, int cx, int cy, RadialAction action, boolean selected, int slotNum) {
        ...
        fill(poseStack, x, y, x + size, y + size, bgColor);

        // Border
        int borderColor = selected ? 0xFFFFFFFF : 0xFF666666;
        fill(poseStack, x - 1, y - 1, x + size + 1, y, borderColor);
        fill(poseStack, x - 1, y + size, x + size + 1, y + size + 1, borderColor);
        fill(poseStack, x - 1, y, x, y + size, borderColor);
        fill(poseStack, x + size, y, x + size + 1, y + size, borderColor);

        // Selection glow
        if (selected) {
            fill(poseStack, x - 2, y, x - 1, y + size, 0xFF4488FF);
            fill(poseStack, x + size + 1, y, x + size + 2, y + size, 0xFF4488FF);
            fill(poseStack, x, y - 2, x + size, y - 1, 0xFF4488FF);
            fill(poseStack, x, y + size + 1, x + size, y + size + 2, 0xFF4488FF);
        }

        // Draw label
        String label = getActionLabel(action);
        drawCenteredString(poseStack, font, label, cx, cy - 4, 0xFFFFFFFF);
    }
