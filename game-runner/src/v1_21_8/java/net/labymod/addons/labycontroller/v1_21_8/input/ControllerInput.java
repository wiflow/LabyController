...
public class ControllerInput extends ClientInput {
    ...
    // Threshold for converting analog to digital (keyboard-like)
    ...
    public void tick() {
        ...
        float leftRaw = -input.getMoveStrafe(); // Negate to fix inversion
        ...
        // Set move vector (already digital 0/1 values)
        ...
        if (useToggleSprint) {
            ...
            // Auto-disable sprint when not moving forward
            if (sprintToggled && forward <= 0) {
                sprintToggled = false;
            }
