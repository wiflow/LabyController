...
public class RadialMenuScreen extends Screen {
    ...
    private RadialAction[] getDefaultActions() {
        return new RadialAction[] {
            RadialAction.HOTBAR_1,
            RadialAction.HOTBAR_2,
            RadialAction.HOTBAR_3,
            RadialAction.HOTBAR_4,
            RadialAction.HOTBAR_5,
            RadialAction.HOTBAR_6,
            RadialAction.HOTBAR_7,
            RadialAction.HOTBAR_8
        };
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        // Draw very light semi-transparent background (40% opacity)
        ...
        for (int i = 0; i < SLOT_COUNT; i++) {
            ...
            drawSlot(graphics, (int) x, (int) y, actions[i], isSelected, i + 1);
        }

        // Draw center indicator
        int indicatorSize = (int) (8 * animationProgress);
        graphics.fill(centerX - indicatorSize, centerY - indicatorSize,
                     centerX + indicatorSize, centerY + indicatorSize, 0xFFFFFFFF);

        // Draw selected action name at bottom
        if (selectedSlot >= 0 && selectedSlot < actions.length) {
            String name = actions[selectedSlot].getDisplayName();
            graphics.drawCenteredString(font, name, centerX, centerY + (int) RADIUS + 30, 0xFFFFFFFF);
        }
    ...
    private void drawSlot(GuiGraphics graphics, int cx, int cy, RadialAction action, boolean selected, int slotNum) {
        int halfSize = (int) (BUTTON_SIZE / 2);
        int x = cx - halfSize;
        int y = cy - halfSize;
        int size = (int) BUTTON_SIZE;

        // Background (more transparent)
        int bgColor = selected ? 0xCC2266CC : 0xAA333333;
        ...
        // Draw slot number or action label
        String label = getActionLabel(action);
        graphics.drawCenteredString(font, label, cx, cy - 4, 0xFFFFFFFF);
    }
    ...
    private String getActionLabel(RadialAction action) {
        ...
        return switch (action) {
            ...
            case HOTBAR_8 -> "8";
            case HOTBAR_9 -> "9";
            case TOGGLE_PERSPECTIVE -> "CAM";
            case DROP_ITEM -> "DROP";
            case DROP_STACK -> "STACK";
            case OPEN_CHAT -> "CHAT";
            case OPEN_COMMAND -> "CMD";
            case TOGGLE_HUD -> "HUD";
            case NONE -> "-";
        };
    }
    ...
    private void updateSelection() {
        ...
        float stickX = state.getRightStickX();
        float stickY = state.getRightStickY();
        ...
        // Offset so slot 0 is at top (270 degrees)
        degrees = (degrees + 90) % 360;

        // Calculate which slot
        int slot = (int) (degrees / (360.0 / SLOT_COUNT));
        selectedSlot = Math.max(0, Math.min(SLOT_COUNT - 1, slot));
    }

        public void executeSelection() {
        if (selectedSlot >= 0 && selectedSlot < actions.length) {
            executeAction(actions[selectedSlot]);
        }
        onClose();
    }

    private void executeAction(RadialAction action) {
        if (action == null || action == RadialAction.NONE) {
            return;
        }
        ...
        switch (action) {
            case HOTBAR_1 -> mc.player.getInventory().setSelectedSlot(0);
            case HOTBAR_2 -> mc.player.getInventory().setSelectedSlot(1);
            case HOTBAR_3 -> mc.player.getInventory().setSelectedSlot(2);
            case HOTBAR_4 -> mc.player.getInventory().setSelectedSlot(3);
            case HOTBAR_5 -> mc.player.getInventory().setSelectedSlot(4);
            case HOTBAR_6 -> mc.player.getInventory().setSelectedSlot(5);
            case HOTBAR_7 -> mc.player.getInventory().setSelectedSlot(6);
            case HOTBAR_8 -> mc.player.getInventory().setSelectedSlot(7);
            case HOTBAR_9 -> mc.player.getInventory().setSelectedSlot(8);
            case TOGGLE_PERSPECTIVE -> {
                var perspective = mc.options.getCameraType();
                mc.options.setCameraType(perspective.cycle());
            }
            case DROP_ITEM -> mc.player.drop(false);
            case DROP_STACK -> mc.player.drop(true);
            case OPEN_CHAT -> {
                // Schedule chat opening after this screen closes
                mc.execute(() -> mc.setScreen(new net.minecraft.client.gui.screens.ChatScreen("")));
            }
            case OPEN_COMMAND -> {
                // Schedule command chat opening after this screen closes
                mc.execute(() -> mc.setScreen(new net.minecraft.client.gui.screens.ChatScreen("/")));
            }
