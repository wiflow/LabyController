package net.labymod.addons.labycontroller.v1_21_8.keyboard;
...
public class OnScreenKeyboard extends Screen {
    ...
    // Keyboard layout matching Controlify style
    ...
    private EditBox targetEditBox;  // Not final - gets refreshed after parent init
    ...
    protected void init() {
        ...
        // Calculate keyboard dimensions first
        int keyHeight = 36;
        int padding = 2;
        keyboardHeight = (KEYBOARD_LAYOUT.length * (keyHeight + padding)) + padding + 4;
        keyboardY = this.height - keyboardHeight;

        // Re-init parent screen
        if (parentScreen != null) {
            parentScreen.init(minecraft, width, height);

            // IMPORTANT: Re-extract EditBox and CommandSuggestions after init
            // because ChatScreen creates NEW instances during init()
            ...
            // Make sure the EditBox has focus for suggestions to work
    ...
    private void insertChar(String str) {
        ...
        // Trigger the EditBox's value change listener (if any)
        // This is important for command suggestions to work
        String text = targetEditBox.getValue();

        // Trigger suggestion update - this sends a packet to the server for completions
        if (commandSuggestions != null) {
            try {
                // Allow suggestions to show
            ...
            // Force the suggestions to update
    ...
    private void doBackspace() {
        ...
        // Update command suggestions after deleting
        if (commandSuggestions != null) {
            commandSuggestions.updateCommandInfo();
        }
    }
    ...
    private void doEnter() {
        if (parentScreen instanceof ChatScreen && targetEditBox != null) {
            ...
            if (!message.isEmpty()) {
                // Use reflection to call handleChatInput directly
                // This is more reliable than keyPressed since we bypass screen context issues
                try {
                    ...
                    // Fallback: try sending via connection directly
    ...
    public void tick() {
        ...
        // Update command suggestions if typing a command
        if (commandSuggestions != null && targetEditBox != null) {
            String text = targetEditBox.getValue();
            if (text.startsWith("/")) {
                // Trigger suggestion update by simulating key input
                commandSuggestions.updateCommandInfo();
            }
        }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        // Draw semi-transparent background for the entire screen
        graphics.fill(0, 0, this.width, this.height, 0x88000000);

        // Input field area (above keyboard)
        ...
        if (targetEditBox != null) {
            ...
            // Render text with slight padding
        ...
        // Render command suggestions above the input field
        ...
        // Draw button legend at top of screen
        renderButtonLegend(graphics);

        // Draw hint text below button legend
    ...
    private String getButtonLabel(String xboxButton) {
        if (controllerType == ControllerType.PLAYSTATION) {
            ...
            // Switch has A/B and X/Y swapped visually
            return switch (xboxButton) {
                case "A" -> "B";
                case "B" -> "A";
                case "X" -> "Y";
        ...
        return xboxButton; // Xbox/Generic
    }

    private int getButtonColor(String xboxButton) {
        if (controllerType == ControllerType.PLAYSTATION) {
            return switch (xboxButton) {
                case "A" -> 0xFF6888C8;  // Cross - Light blue
                case "B" -> 0xFFE86A6A;  // Circle - Red/Pink
                case "X" -> 0xFFD88AD8;  // Square - Pink/Magenta
                case "Y" -> 0xFF68C8A8;  // Triangle - Teal/Green
        ...
        // Xbox colors
        return switch (xboxButton) {
            case "A" -> 0xFF22AA22;  // Green
            case "B" -> 0xFFCC3333;  // Red
            case "X" -> 0xFF2266CC;  // Blue
            case "Y" -> 0xFFCC9900;  // Yellow
            case "LB", "RB" -> 0xFF555555;
            default -> 0xFF555555;
        };
    }
    ...
    private void renderKey(GuiGraphics graphics, KeyData key, boolean selected) {
        ...
        if (key.shortcut != null) {
            // Has shortcut - draw controller button icon + text
            int iconX = x + 8;
            int iconY = y + (h - 14) / 2;

            // Convert shortcut to controller-specific label
    ...
    private void renderButtonIcon(GuiGraphics graphics, String label, int color, int x, int y) {
        ...
        if (label.equals("LB") || label.equals("RB")) {
            // Bumper style - rounded rectangle
            int w = 20;
            ...
            // Face button style - circular
            // Draw a more circular shape
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        // Pass navigation keys to parent for suggestions
